...
rule fake_dropseq:
    input:
        rules.cut_round2.output.paired_output
    output:
        os.path.join(ana_home, "RNA_cc", "{sample}_R2.fq.gz")
    threads: 1
    resources: nodes = 1
    log: log_path("fake_dropseq.log")
    message: "---> fake_dropseq : {wildcards.sample} : {threads} cores"
    run:
        CB = ID2CB(wildcards.sample)
